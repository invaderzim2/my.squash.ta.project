...
//    baseUrl = "https://www-uat.sw.co.ua/";
  ...
  public void testRatings() throws Exception {
	  try {
		  ...
		  Report.WriteToLog("Open ratings tab");
		  ...
		  Report.WriteToLog("Check first element presents");
		  
		  Thread.sleep(1000);
		  
		  for (int second = 0;; second++) {
			  	if (second >= 20) fail("timeout");
			  	try { if (isElementPresent(By.xpath("//div[@class='innerResult']/div[1]"))) break; } catch (Exception e) {}
			  	Thread.sleep(1000);
			  }
		  
		  Report.WriteToLog("Check last element presents");
		  
		  Thread.sleep(1000);

		  assertTrue(isElementPresent(By.xpath("//div[@class='innerResult']/div[100]")));
