...
//    baseUrl = "https://www-uat.sw.co.ua/";
  ...
  public void testGroup() throws Exception {
	  try {
		  ...
		  Report.WriteToLog("Open Group interface");

		  for (int second = 0;; second++) {
		  	if (second >= 60) fail("timeout");
		  	try { if (isElementPresent(By.cssSelector("div.group"))) break; } catch (Exception e) {}
		  	Thread.sleep(1000);
		  }
		  
		  driver.findElement(By.cssSelector("div.group")).click();

		  Thread.sleep(1000);
		  
		  for (int second = 0;; second++) {
			  	if (second >= 20) fail("timeout");
			  	try { if (isElementPresent(By.id("group"))) break; } catch (Exception e) {}
			  	Thread.sleep(1000);
			  }
