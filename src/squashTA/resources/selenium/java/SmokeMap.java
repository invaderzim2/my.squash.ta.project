...
//    baseUrl = "https://www-uat.sw.co.ua/";
  ...
  public void testMap() throws Exception {
	  try {
		  ...
		  Report.WriteToLog("Open map");

		  for (int second = 0;; second++) {
		  	if (second >= 60) fail("timeout");
		  	try { if (isElementPresent(By.id("openMap"))) break; } catch (Exception e) {}
		  	Thread.sleep(1000);
		  }
		  ...
		  Report.WriteToLog("Wait till map loaded");

		  for (int second = 0;; second++) {
		  	if (second >= 60) fail("timeout");
		  	try { if (isElementPresent(By.id("mapsCanvas"))) break; } catch (Exception e) {}
		  	Thread.sleep(1000);
		  }
